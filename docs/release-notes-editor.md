...
- Diese Regeln werden jetzt überall geprüft, wo eine ID oder ein Alias eingegeben wird — auch bei Zustandsvariablen, Ziehelementen, Zeilen der Optionentabelle und verfolgten Variablen eines Geometrie-Elements. Eine unerlaubte Eingabe wird nicht übernommen und benannt; bringt eine geladene Aufgabe solche Namen mit, nennt ein Hinweis sie, und sie werden nicht als Variablen gemeldet
...
### Fehlerbehebungen
- IDs und Aliase: Behebt Fehler bei der Vergabe, insbesondere beim Duplizieren und beim Einfügen von Abschnitten. Die IDs gelöschter Elemente werden wieder frei — auch die der Elemente in einem Lückentext, einer Tabelle oder einer Optionentabelle, und die eines Abschnitts, dessen Einfügen abgelehnt wurde. Zustandsvariablen können nicht mehr die ID oder den Alias eines Elements bekommen
- Aufgabenwechsel im Studio: Der Initialisierungsfehler beim schnellen Wechsel (z. B. doppelte IDs) tritt nicht mehr auf. Ein Hinweis oder eine Rückfrage, die beim Wechsel noch offen steht, wird mit der alten Aufgabe verworfen und kann die neue nicht mehr verändern. Lädt der Host eine leere Aufgabe nach, wird die Auswahl zurückgesetzt
- Auswahl: Gelöschte Elemente sind nicht mehr ausgewählt, auch nicht die Kinder eines gelöschten Lückentexts oder einer Tabelle. Bisher bot der Eigenschaftenbereich weiter deren Einstellungen an; Änderungen daran gingen ins Leere, die Aufgabe wurde aber als geändert gemeldet
- Löschen: Die Entf-Taste löscht kein ausgewähltes Kind eines zusammengesetzten Elements mehr (Lücke im Lückentext, Tabellenzelle) — diese werden weiterhin im Bearbeitungsdialog ihres Elternelements entfernt. Ist ein solches Element mitausgewählt, ist Löschen gesperrt, unabhängig von der Reihenfolge der Klicks; im Übersichtsdialog ist die Schaltfläche in diesen Zeilen ebenfalls gesperrt und ein Hinweis sagt, warum
...
- Übersichtsdialog: Das Öffnen hinterlässt keine Zusatzangaben in der gespeicherten Aufgabe. Eine gelöschte Zeile verlässt die Auswahl — eine anschließende "Mehrfachänderung" schrieb bisher auf ein Element, das es nicht mehr gibt, und meldete die Aufgabe als geändert
...
- Elementinteraktion: Der Schalter wirkt jetzt auch auf die Kinder von Lückentext und Tabelle — ein Klick dorthin wählt das Element aus, statt in das Eingabefeld zu springen. In statisch positionierten Abschnitten ist die Auswahl am gestrichelten Rahmen zu erkennen
- Seiten: "Seitenumbruch entfernen" ist auf der ersten regulären Seite gesperrt, solange es eine dauerhaft sichtbare Seite gibt (bisher ließ sich die Seite darin auflösen). "Seitenumbruch ab gewähltem Abschnitt" ist nur bedienbar, wenn der gewählte Abschnitt auf dieser Seite liegt. Die Seitenliste zeigt die Vorschau des Navigationsknopfes zur nächsten Unit auch dann auf der letzten Seite, wenn es eine dauerhaft sichtbare Seite gibt
- Datei-Import: Schlägt das Laden der Datei fehl (Bild, Audio, Video, Hotspot-Bild, Geometrie), wird kein Element angelegt und eine Meldung gezeigt. Bisher entstand ein Element ohne Datei, das erst gelöscht werden musste
- Duplizieren: Kopie und Original sind jetzt vollständig voneinander unabhängig. Bisher teilten sie an einigen Stellen dieselben Daten — bearbeitete man in der Kopie z. B. die Optionsliste einer Klappliste, änderte sich unbemerkt auch das Original, und rückgängig machen ließ sich das nicht. Die Ziehelemente einer duplizierten Ablegeliste bekommen wieder eigene IDs
- Ältere Aufgaben werden beim Laden vollständiger übernommen: Die Größen von Elementen in Tabellen, Optionentabellen und Lückentexten bleiben erhalten, ebenso die Einstellungen und Ränder der Medien-Elemente
...
- Assistenten
  - Korrigiert die Element-Auswahl nach dem Einfügen zweiseitiger Abschnittsvorlagen
...
- Mehrfachauswahl
  ...
  - Sechs Felder verschwanden bei unterschiedlichen Werten ganz und waren für die gemeinsame Bearbeitung unerreichbar (u. a. "Anzahl der Spalten", "Ausrichtung", "Verbundene Ablagelisten"); sie lassen sich jetzt für die ganze Auswahl setzen
  - Bei Grenzen (Mindest- und Maximalbreite, Mindest- und Maximalhöhe, "Elemente je Zeile begrenzen", Vorbelegung des Schiebereglers) ist zu erkennen, ob es keine Grenze gibt oder verschiedene; das Feld ist sofort bedienbar, und das Anhaken überschreibt die Werte nicht mehr mit einem Standardwert
