...
- Neuer Dialog zum Upload von Bildern
  ...
  - Breite und Höhe sind fest gekoppelt: Die eine Angabe rechnet die andere mit, das Seitenverhältnis bleibt immer erhalten. Die Felder starten mit der Größe, die das Bild nach dem Hochladen tatsächlich hat
...
- Lückentext
  - Im Texteditor des Lückentexts fügt der neue Knopf "Klappliste" eine Klappliste als Lücke ein — neben den bisherigen Lückentypen Eingabefeld, Ablegeliste, Optionsfeld, Kontrollkästchen und Knopf. Ihre Optionen und die Vorbelegung werden wie bei einer freistehenden Klappliste im Eigenschaftenbereich eingestellt
...
- Diese Regeln werden jetzt überall geprüft, wo eine ID oder ein Alias eingegeben wird: bei Elementen, Zustandsvariablen, den IDs der Ziehelemente einer Ablegeliste, den Zeilen der Optionentabelle und den verfolgten Variablen eines Geometrie-Elements. Eine unerlaubte Eingabe wird nicht übernommen; der Dialog nennt den Grund und lässt sich nicht speichern. Bisher gab es diese Prüfung nur an einigen dieser Stellen, sodass Aufgaben mit Variablennamen entstehen konnten, die die Kodierung nicht verarbeiten kann. Enthält eine geladene Aufgabe noch solche Namen, nennt ein Hinweis sie beim Namen — an den Host gemeldet werden sie nicht
- Vier Einstellungen sind aus dem Eigenschaftenbereich entfernt, weil sie nie eine Wirkung hatten: "Anteil der ersten Spalte" bei der Zeile einer Optionentabelle (das Raster kommt aus der Tabelle selbst), "Beschriftung" bei Bildbereichen und bei der Zeile einer Optionentabelle, die Hintergrundfarbe bei "Optionsfelder mit Bild" sowie im Expertenmodus "Knopf zum Leeren anzeigen" beim einfachen Eingabefeld. Gespeicherte Werte werden beim Laden verworfen; die Darstellung ändert sich dadurch nicht
- Sind Elemente unterschiedlichen Typs gemeinsam markiert, bietet der Eigenschaftenbereich nur noch Einstellungen an, die alle von ihnen wirklich haben. Bisher konnte eine solche Einstellung erscheinen, weil ein Element des Typs sie besitzt — geschrieben wurde sie dann nur bei diesem. Bei einer gemischten Auswahl aus Knopf und Auslöser gibt es deshalb keine gemeinsame Aktionsauswahl mehr: Die möglichen Aktionen der beiden Typen unterscheiden sich
- Assistent "Geometrie": Der erzeugte Quellenhinweis nennt nur noch GeoGebra samt Lizenzbedingungen; der IQB-Teil entfällt, weil die Quelle künftig auf der Ende-Seite steht
- Formelfeld
  - UI-Verbesserung der Eingabemöglichkeit zur Vorbelegung: Ein Knopf schaltet zwischen der Formel-Eingabe und dem LaTeX-Quelltext um, sodass eine Vorbelegung auch direkt als LaTeX geschrieben oder eingefügt werden kann  
- Texteditor
...
- Wechselt man im Studio zu einer anderen Aufgabe, während eine Löschabfrage (Seite, Abschnitt oder Elemente) noch offen steht, wird die Abfrage verworfen. Bisher wurde nach dem Bestätigen an derselben Stelle der neu geladenen Aufgabe gelöscht. Außerdem wird eine abgebrochene Löschung eines Abschnitts nicht mehr als Änderung der Aufgabe gemeldet
- Nach dem Einfügen eines Abschnitts über "Seitenabschnitt einfügen" ist der neue Abschnitt ausgewählt, auch beim Ersetzen eines bestehenden. Bisher sprang die Auswahl eine Stelle nach oben (beim Ersetzen zwei), sodass der Eigenschaftenbereich einen anderen Abschnitt zeigte
- Lehnt man beim Ersetzen eines Abschnitts die Rückfrage ab, sind die IDs des nicht eingefügten Abschnitts wieder frei. Bisher blieben sie für den Rest der Sitzung belegt, ohne dass ein Element sie trug — beim nächsten Einfügen desselben Abschnitts fragte der Editor deshalb erneut nach neuen IDs. Dasselbe galt, wenn der Host die Aufgabe austauschte, während die Rückfrage offen stand
- Beim Ersetzen eines Abschnitts über "Seitenabschnitt einfügen" wird nicht mehr nachgefragt, wenn der bestehende Abschnitt leer ist — dort geht nichts verloren. Lehnt man die Rückfrage bei einem Abschnitt mit Inhalt ab, bleibt dieser wie bisher erhalten; ein Hinweis sagt jetzt, dass der einzufügende Abschnitt damit verworfen wurde. Bisher geschah in diesem Fall wortlos nichts
...
- Der Übersichtsdialog schreibt keine Zusatzangaben mehr in die Aufgabe. Bisher hinterließ jedes Öffnen an allen Elementen zwei interne Zahlen (die Seite und der Abschnitt, in dem sie liegen), die anschließend mitgespeichert wurden — und veraltet waren, sobald ein Abschnitt verschoben wurde
- Löscht man im Übersichtsdialog eine angekreuzte Zeile, ist sie auch aus der Auswahl heraus. Bisher blieb sie darin: Eine anschließende "Mehrfachänderung" schrieb auf ein Element, das es nicht mehr gibt, meldete die Aufgabe als geändert, obwohl sich nichts geändert hatte, und die Kopf-Auswahl zählte die Zeile weiter mit. Gilt auch für die Zeilen der Kinder eines gelöschten zusammengesetzten Elements
...
- In der Elementliste eines Abschnitts löst das Überfahren einer Zeile einer Optionentabelle oder einer Tabellenzelle keinen Fehler mehr aus. Solche Zeilen stehen weiterhin nur zur Orientierung in der Liste und lassen sich nicht auswählen — jetzt auch sichtbar gesperrt und eingerückt, wie schon bisher die Lücken eines Lückentexts. Außerdem bleibt eine Hervorhebung auf der Arbeitsfläche nicht mehr stehen, wenn die Liste geschlossen wird, während der Zeiger auf einer Zeile steht
...
- Ist die Einstellung "Navigationsknopf zur nächsten Unit anfügen" aktiv, zeigt die Seitenliste seine Vorschau auch dann auf der letzten Seite, wenn es eine dauerhaft sichtbare Seite gibt. In diesen Aufgaben fehlte sie bisher auf allen Seiten, während der Player den Knopf auf der letzten Seite anzeigte
- Schlägt beim Anlegen eines Bild-, Audio-, Video-, Hotspot-Bild- oder Geometrie-Elements das Laden der Datei fehl, wird kein Element mehr angelegt und eine Meldung gezeigt. Bisher entstand ein Element ohne Datei und ohne Hinweis, das erst gelöscht werden musste, bevor der Import erneut versucht werden konnte. Bricht man den Dialog ab, entsteht wie bisher kein Element — und weiterhin ohne Meldung
...
- Beim Laden einer Aufgabe im Format vor 4.0 werden jetzt auch die Elemente innerhalb von Tabellen, Optionentabellen und Lückentexten umgewandelt. Ihre gespeicherte Größe bleibt damit erhalten — im Eigenschaftenbereich sichtbar, bei den Lücken eines Lückentexts als Mindestbreite auf der Arbeitsfläche. Bisher bekamen diese Elemente die Standardgröße ihres Typs: eine Lücke mit gespeicherten 150 Pixeln Breite wurde 180 Pixel breit, eine Tabellenzelle mit 333x111 zeigte im Eigenschaftenbereich 180x98. Aufgaben, die seither im Editor geöffnet und gespeichert wurden, tragen den Standardwert dauerhaft; für sie kommt die Korrektur zu spät
...
- Der Knopf "Seitenumbruch entfernen" ist auf der ersten regulären Seite gesperrt, solange es eine dauerhaft sichtbare Seite gibt. Bisher ließ sich die erste reguläre Seite damit in die dauerhaft sichtbare Seite auflösen: ihre Abschnitte wurden von da an neben jeder Seite angezeigt, und bei zwei Seiten blieb eine Aufgabe zurück, die nur noch aus der dauerhaft sichtbaren Seite bestand
- Der Knopf "Seitenumbruch ab gewähltem Abschnitt" einer Seite ist nur noch bedienbar, wenn der gewählte Abschnitt auf dieser Seite liegt. Bisher waren die Knöpfe aller Seiten bedienbar: drückte man den Knopf einer anderen Seite, wurde kein Abschnitt verschoben, es entstand eine Seite ohne Abschnitte, und der Editor meldete einen Fehler
...
- Duplizieren von Elementen und Abschnitten: Kopie und Original arbeiteten an einigen Stellen auf denselben Daten. Bearbeitete man in der Kopie z. B. die Optionsliste einer Klappliste, änderte sich unbemerkt auch das Original — und rückgängig machen ließ sich das nicht. Kopie und Original sind jetzt vollständig voneinander unabhängig. Betroffen waren die Optionslisten von Klappliste, Optionsfeldern (auch mit Bild) und Umschaltknöpfen, die Bereiche von Hotspot-Bildern, die Tastatur-Vorgaben von Formelfeld und Formelbereich, die Beschriftung von Likert-Zeilen, die Markierpanel-Zuordnung von Textelementen, die verfolgten Variablen von Geogebra-Elementen, die Spalten- und Zeilengrößen von Tabellen und Abschnitten, die Sichtbarkeitsregeln von Abschnitten sowie die Ränder aller Elemente. Das Kopieren und Einfügen über die Zwischenablage war nicht betroffen
...
- Tabelle
  ...
  - Der Dialog "Elemente anpassen" arbeitet auf einer Kopie: "Abbrechen" nimmt jetzt zurück, was darin geändert wurde. Bisher wirkte jede Änderung sofort auf die Aufgabe — eine im Dialog entfernte Zelle war weg und ließ sich mit "Abbrechen" nicht zurückholen, eine hinzugefügte blieb nach dem Abbrechen stehen. Die IDs entfernter Zellen werden erst beim Speichern frei
- Assistenten
  - Korrigiert die Element-Auswahl nach dem Einfügen zweiseitiger Abschnittsvorlagen aus einem nachfolgenden Abschnitt
  - Im Drag-&-Drop-Assistenten, Variante "Sortieren", wird die eingegebene Überschrift der Elementliste jetzt auch angelegt. Bisher wurde der Text ohne Hinweis verworfen, während die Varianten "Zuordnung" ihn setzen
  - Im Stimulus-Assistenten "E-Mail" steht in der französischen Fassung vor den Doppelpunkten von "De", "À" und "Objet" ein schmales geschütztes Leerzeichen, wie es die französische Typografie verlangt
...
- Zahlenfelder im Eigenschaftenbereich
  - Ein geleertes Zahlenfeld (z. B. "X Position", "Z-Index", "Minimalwert"/"Maximalwert" des Schiebereglers, "Größe der Lupe" sowie "Breite" und "Höhe") schreibt keinen ungültigen Leerwert mehr in die Aufgabendatei: die Eingabe wird beim Verlassen des Feldes verworfen, das Feld zeigt wieder den gespeicherten Wert und es erscheint der Hinweis "Eingabe ungültig". Wo eine Zahl verlangt ist, ist ein leeres Feld damit keine Eingabe, sondern eine unvollständige
  - Felder für eine Ober- oder Untergrenze ("Mindestbreite"/"Maximalbreite", "Mindesthöhe"/"Maximalhöhe", "Minimallänge"/"Maximallänge", "Vorbelegung" des Schiebereglers) dürfen dagegen weiterhin leer bleiben: dort bedeutet ein leeres Feld "keine Grenze" und wird auch so gespeichert. Eine einmal gesetzte Grenze lässt sich damit wieder entfernen
  - "Zeilenspanne" und "Spaltenspanne" im Raster nehmen keine 0 mehr an: 0 wurde vom Layout ohnehin wie 1 dargestellt, sodass gespeicherter Wert und Darstellung auseinanderfielen
...
- Zahlenfelder außerhalb des Eigenschaftenbereichs
  ...
  - Eine Eingabe mit Enter zu bestätigen wirkt jetzt genauso wie das Verlassen des Feldes: eine Zahl, die eingegeben und wieder gelöscht wurde, wird nicht mehr gespeichert, das Feld zeigt wieder den gespeicherten Wert und der Hinweis "Eingabe ungültig" erscheint. Bisher wurde die gelöschte Zahl mit Enter übernommen, ohne Hinweis — bei den Maßfeldern stand damit ein Wert in der Aufgabendatei, der im Feld gar nicht mehr zu sehen war, und bei der Zeilen- und Spaltenanzahl von Tabelle und Abschnitt sowie bei der Anzahl der Antwortfelder im Assistenten gingen dadurch Zeilen, Spalten bzw. Antwortfelder samt ihren Inhalten verloren
- Voreinstellungen neuer Elemente: Beim Umbau auf die gemeinsame Vorgabentabelle sind Werte verlorengegangen, die in der produktiven Version 2.12.5 galten. Sie sind wiederhergestellt. Betroffen waren: die Bildschirmtastatur samt "Eingabehilfe in Tastatur übernehmen" und "Gerätetastatur unterdrücken" bei Eingabefeld, einfachem Eingabefeld, Eingabebereich, "Wort korrigieren" und Formel-Eingabebereich (dort waren alle drei Häkchen aus statt an), die Höhe und Zeilenhöhe der Optionsfelder, die Breite des Audio-Elements, die Zeilenhöhe des Schiebereglers (5 % statt 135 %, dadurch überlappende Beschriftung), "Platzhalter mit Durchschrift" bei Ablegelisten sowie Breite, feste Breite und Zeilenhöhe der Eingabefelder im Lückentext. Bereits gespeicherte Aufgaben behalten ihre Werte; die Änderung betrifft nur neu angelegte Elemente
- Schieberegler: Die beiden Anzeigeoptionen "Balken-Darstellung" und "Werteanzeige am Griff" sind bei neuen Elementen wieder ausgeschaltet. In den 3.0.0-Betas waren sie versehentlich an — nicht durch eine Entscheidung, sondern weil bei der Umstellung auf den neuen Migrationsmechanismus statt der Schalter (aus/an) zwei Textwerte eingetragen wurden, die immer als "an" gelesen werden. Die produktive Version 2.12.5 war nicht betroffen
- Mehrfachauswahl: abweichende Werte sind jetzt erkennbar
  ...
  - Auch Felder, in denen ein leerer Wert selbst eine gültige Angabe ist, unterscheiden jetzt zwischen "bei allen leer" und "unterschiedlich": Mindest- und Maximalbreite, Mindest- und Maximalhöhe, "Elemente je Zeile begrenzen" und die Vorbelegung des Schiebereglers. Sind die Werte unterschiedlich, zeigt das zugehörige Kontrollkästchen seinen Zwischenzustand, das Feld ist von Anfang an bedienbar und trägt den Hinweis "Werte unterschiedlich". Bisher sah eine Auswahl mit unterschiedlichen Grenzen aus wie eine ohne jede Grenze, und das Feld war zunächst gesperrt — man musste das Kästchen erst anhaken, um überhaupt eine gemeinsame Grenze eingeben zu können, ohne zu sehen, dass es dort schon Grenzen gab. Bei "Elemente je Zeile begrenzen" überschrieb dieses Anhaken die unterschiedlichen Werte zudem sofort mit dem Standardwert 4
  - Auch die Vorbelegung wird jetzt gekennzeichnet, wenn sie sich unterscheidet — bei Eingabefeld, Eingabebereich, Auswahlliste, Auswahlknöpfen und Formelfeld. Bisher war dort nicht zu erkennen, ob keines der markierten Elemente eine Vorbelegung hat oder ob sie verschieden sind. Bei der Auswahlliste steht das Zeichen am linken Feldrand, beim Formelfeld neben der Beschriftung "Vorbelegung" (in der Formel- wie in der LaTeX-Ansicht)
  - Ohne Kennzeichnung bleibt "keine Aktion" bei Knopf und Auslöser: dort steht das leere Feld weiterhin für beides
...
- Fehlerdialog "Unerwarteter Fehler": Trat ein Fehler auf, der beim Aufbau der Arbeitsfläche immer wieder auftritt, öffnete sich der Dialog endlos neu — auch "Verwerfen" brachte ihn sofort zurück, und der Editor war nicht mehr bedienbar, sodass die Aufgabe nicht einmal mehr gespeichert werden konnte. Ursache war der Dialog selbst: sein Öffnen und Schließen lässt die Arbeitsfläche neu zeichnen, was denselben Fehler wieder auslöste. Jeder Fehler wird jetzt höchstens einmal gemeldet, danach bleibt der Editor bedienbar; ein anderer Fehler bekommt weiterhin seinen eigenen Dialog. "Höchstens einmal" gilt je geladener Aufgabe: sobald eine Aufgabe geladen wird — eine andere oder dieselbe erneut —, wird derselbe Fehler dort wieder gemeldet; steht der Dialog der vorherigen Aufgabe noch offen, erst nach dem Verwerfen. Auch in der Browser-Konsole steht ein wiederkehrender Fehler jetzt nur noch einmal je Aufgabe statt endlos oft
- Die vier Knöpfe unter "Ausrichtung" im Eigenschaftenbereich richten mehrere markierte Elemente jetzt tatsächlich aneinander aus (links, rechts, oben, unten). Bisher bewirkte ein Klick nichts: Die Position wurde an der falschen Stelle im Element abgelegt, sodass die Elemente stehen blieben und die Aufgabe eine überzählige Angabe mitspeicherte
- Verwirft man eine Aufgabe, die noch nie mit Inhalt gespeichert wurde, und lädt der Host daraufhin die leere Definition nach, öffnet sich kein endloser Fehlerdialog mehr. Bisher blieben Abschnitts- und Elementauswahl auf Stellen stehen, die es in der leeren Aufgabe nicht gibt, woraufhin der Eigenschaftenbereich bei jedem Zeichnen denselben Fehler auslöste
- Der Schalter "Elementinteraktion erlauben" wirkt jetzt auch auf die Kinder von Lückentext und Tabelle. Bisher waren diese immer bedienbar, unabhängig vom Schalter: Ein Klick in eine Tabellenzelle oder eine Lücke setzte den Cursor ins Eingabefeld, statt das Element auszuwählen. Außerdem ist in statisch positionierten Abschnitten jetzt auch dort am gestrichelten Rahmen zu erkennen, welches Element ausgewählt ist
- Ändert man einen Text oder ein Lückentext-Dokument so, dass Referenzen ungültig werden, wird die Aufgabe mit dem Wert gemeldet, der nach der Rückfrage tatsächlich geschrieben wurde. Bisher ging die Meldung schon während der offenen Rückfrage hinaus, mit dem alten Wert: Die im Editor entfernte Lücke stand beim Host weiter in der Definition, bis irgendeine spätere Änderung die Aufgabe erneut meldete — wurde zwischenzeitlich gespeichert, gewann der veraltete Stand
- Duplizieren, Verschieben und die Positionsfelder treffen den Abschnitt, in dem das ausgewählte Element liegt. Bisher lasen diese Wege den intern gemerkten Abschnitts-Index, der auf einen anderen Abschnitt zeigen konnte — nach dem Laden einer Aufgabe stand er auf dem ersten Abschnitt, während das zuletzt aufgebaute Element ausgewählt war. Ein Duplikat landete dann im falschen Abschnitt, die Tab-Reihenfolge wurde im falschen Abschnitt sortiert, und der Eigenschaftenbereich zeigte Rasterfelder für ein Element in einem statisch positionierten Abschnitt (und umgekehrt). Ein Klick in einen statisch positionierten Abschnitt merkt außerdem die Seite mit, was in der Listenansicht aller Seiten fehlte
