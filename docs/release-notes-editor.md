...
## 3.0.1
### Neue Funktionen
- Bilder lassen sich nachträglich verkleinern: Der Dialog zum Verkleinern, den bisher nur das Hochladen öffnete, ist jetzt auch für ein Bild erreichbar, das schon in der Aufgabe steckt — über einen eigenen Knopf neben dem Bild. Es gibt ihn bei Bild und Bildbereichen im Eigenschaftenbereich, bei Knopf und Kontrollkästchen mit Bild, beim Startbild der Abspielsteuerung und in jedem Dialog, in dem eine Beschriftung mit Bild bearbeitet wird (Optionsfelder, Klappliste, Ablegelistenoption, Zeilen und Spalten der Optionentabelle). Der Dialog weist dabei darauf hin, dass das Bild neu berechnet wird und ein JPEG oder WebP dabei ein zweites Mal an Qualität verliert. Bei Bildern, die sich gar nicht verkleinern lassen — etwa SVG —, ist der Knopf gesperrt und nennt den Grund
- Der Qualitätsregler im Verkleinern-Dialog wirkt jetzt auch dann, wenn die Maße unverändert bleiben. Bisher gab der Dialog ein Bild unverändert zurück, sobald weder Breite noch Format geändert wurden — der Regler entschied dann nichts. Ein Bild, das nur zu groß in Bytes, aber richtig in den Maßen ist, lässt sich damit erstmals verkleinern. Ein Upload, den man nur bestätigt, ohne den Regler anzufassen, wird weiterhin unverändert übernommen
- Bildbereiche: Werden beim Verkleinern die Maße geändert, warnt der Dialog — die Bildbereiche liegen an festen Positionen im Bild und wandern nicht mit. Nur die Dateigröße zu senken, ohne die Maße anzufassen, lässt sie unberührt

### Änderungen
- Zahlenfelder, in denen eine eingegebene 0 nicht "keins" bedeutet, sagen es jetzt in einem Hinweis unter dem Feld: minimale und maximale Anzahl der Durchläufe im Dialog "Abspielsteuerung" (kein Durchlauf verpflichtend bzw. unbegrenzt oft abspielbar), Minimal- und Maximallänge bei Eingabefeld und Eingabebereich sowie Mindest- und Maximalbreite und -höhe (keine Begrenzung, wie ein leeres Feld). Beim Periodensystem sagt "Max. Anzahl Auswahlen" jetzt, dass 0 unbegrenzt viele Auswahlen erlaubt. Die Beschriftung "Minimale Anzahl der Durchläufe (verpflichtend)" sagte das Gegenteil dessen, was eine 0 dort bewirkt, und heißt jetzt "Minimale Anzahl der Durchläufe"

### Fehlerbehebungen
- Bilder verkleinern: Das verkleinerte Bild ist feiner. Bisher überließ das Modul das Verkleinern dem Browser, und der las dafür nur vier Quellpixel je Zielpixel — bei einer Verkleinerung auf ein Drittel trug damit rund jeder zehnte Bildpunkt zum Ergebnis bei, der Rest fiel weg. Dünne Linien und kleine Schrift verloren dadurch mehr als nötig, und das Ergebnis wirkte im Vergleich zu einem Bildbearbeitungsprogramm ausgefranst. Das Modul rechnet das Verkleinern jetzt selbst, sodass Chrome, Firefox und Safari praktisch dasselbe Ergebnis liefern — bisher rechnete Safari deutlich gröber als die beiden anderen, und je stärker verkleinert wurde, desto größer war der Abstand. Außerdem werden die Zielmaße gerundet: Ergab die Rechnung eine gebrochene Höhe, wurde das Bild um einen halben Bildpunkt versetzt gezeichnet und die unterste Zeile abgeschnitten. Die Dateigröße ändert sich dabei kaum — gemessen unter 5 % in beide Richtungen. Was das Modul nicht ausgleichen kann: Ein Bild, das genau auf seine Anzeigebreite verkleinert wird, hat keine Pixelreserve und wird auf einem hochauflösenden Bildschirm wieder hochgerechnet; wer es dort scharf braucht, wählt eine größere Breite als die Anzeigefläche
- Zahlenfelder, deren Wert erst beim Verlassen übernommen wird, übernehmen ihn jetzt auch, wenn er über die kleinen Pfeile am Feld verändert wurde: Zeilen- und Spaltenanzahl von Tabelle und Abschnitt, die Felder für Zeilenhöhen, Spaltenbreiten und Ränder sowie die Anzahl der Antworten im Assistenten. Bisher änderte sich nur die Zahl im Feld; in Firefox wurde sie gar nicht übernommen, weil dort der Klick auf die Pfeile den Schreibfokus nicht in das Feld setzt und die Übernahme auf das Verlassen des Feldes wartete
- Texteingabe in Dialogen: Der Editor wächst nicht mehr mit jeder Zeile über den Dialog hinaus. Der Textbereich hält jetzt seine Höhe und scrollt, sodass Werkzeugleiste und die Felder darunter erreichbar bleiben. Der Dialog "Ablegelistenoption anpassen" öffnet außerdem breit genug, dass die Werkzeugleiste vollständig sichtbar ist
- Absatzabstand: Der Abstand lässt sich wieder einstellen. Bisher erschien stattdessen eine Fehlermeldung und der Abstand blieb, wie er war — bei einem Absatz, in dem der Cursor steht, ebenso wie bei einer Auswahl über mehrere
- Die Quellenangabe, die der Assistent für einen GeoGebra-Abschnitt einfügt, endet jetzt mit einem Punkt: "Erstellt mit GeoGebra, https://www.geogebra.org/ (es gelten die GeoGebra-Lizenzbedingungen)."
