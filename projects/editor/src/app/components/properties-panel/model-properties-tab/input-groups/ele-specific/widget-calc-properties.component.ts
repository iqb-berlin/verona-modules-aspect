  ...
  template: `
    <div class="fx-column-start-stretch">
      <mat-form-field appearance="fill">
        <mat-label>{{'propertiesPanel.mode' | translate }}</mat-label>
        <mat-select [value]="$any(combinedProperties).mode"
                    (selectionChange)="updateModel.emit({ property: 'mode', value: $event.value })">
          <mat-option value="SIMPLE">{{ 'propertiesPanel.mode_simple' | translate }}</mat-option>
          <mat-option value="SCIENTIFIC">{{ 'propertiesPanel.mode_scientific' | translate }}</mat-option>
        </mat-select>
      </mat-form-field>

      <mat-form-field appearance="fill">
        <mat-label>{{'propertiesPanel.journalLines' | translate }}</mat-label>
